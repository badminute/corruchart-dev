...
export default function ImportedResultsPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <ImportedResultsContent />
    </Suspense>
  );
}
...
function ImportedResultsContent() {
  ...
  // For imported results, redactions are always locked (read-only)
  const seedRedactionsLocked = true;
  ...
  // TIPS MODAL - disabled for imported results
  ...
  const toggleRedact = (id: string) => {
    if (seedRedactionsLocked) return; // locked when viewing imported results
    setRedactedIds(prev => {
  ...
  const CORRUPTION_MESSAGES: Record<number, string> = {
    0: "You are pure and untouched by corruption.",
  ...
  const rolesBySection = useMemo(() => {
    return ROLE_SECTIONS.map(section => ({
      ...section,
      roles: identityOptions.filter(role =>
  ...
  useEffect(() => {
    ...
    try {
      const { selections: seedSelections, redacted, favorites: seedFavorites } = decodeSelections(seed);

      // Convert selections object to array format
      const userSelections = Object.entries(seedSelections).map(([id, value]) => ({
        id,
        tags: OPTIONS.find(o => o.id === id)?.tags ?? [],
  ...
  // ----------------------------
  // Compute tag scores from selections
  // ----------------------------
  useEffect(() => {
  ...
  const toggleFavorite = (id: string) => {
    // For imported results, favorites are read-only
    return;
  };
  ...
  const exportScreenshot = async () => {
    ...
    // Collect metadata for embedding (use imported data)
    const metadata = {
  ...
  // ----------------------------
  // Generate share URL (works with imported data)
  // ----------------------------
  const generateShareUrl = async () => {
    try {
      ...
      const url = `${window.location.origin}/results?s=${seed}`;
  ...
  const submitForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("https://formsubmit.co/badminute@protonmail.com", {
        method: "POST",
        body: formData,
      });
          ...
          <section className="mt-8 relative">
            ...
            {openTagInfo?.tag === "__fav_help" && (
              <div className="absolute -top-28 left-1/2 transform -translate-x-1/2 z-50 w-64 p-3 bg-neutral-900 text-gray-200 rounded shadow-lg text-center text-sm border border-neutral-700">
                Viewing imported results: pinned items are read-only.
              </div>
            )}
        ...
        <section className="space-y-6 relative">
          ...
          {openTagInfo?.tag === "__positive_help" && (
            <div className="absolute -top-32 left-1/2 transform -translate-x-1/2 z-50 w-64 p-3 bg-neutral-900 text-gray-200 rounded shadow-lg text-center text-sm border border-neutral-700">
              These are the tags from the imported results that were reacted to most positively.
            </div>
          )}
          ...
          <TagAffinityDrilldown
            ...
            seedMode={true} // Always read-only for imported results
            redacted={Array.from(redactedIds)}
          />
